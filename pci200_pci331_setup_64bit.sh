...
# To disable SocketCan driver for PCI200 series boards, add "blacklist plx_pci" in /etc/modprobe.d/blacklist.conf
  ...
  if [ ! -L "/usr/local/sbin/cantest" ]; then
    if [ -f "$HOME/esdcan-pci200-linux-2.6.x-x86_64-3.10.1/bin64/cantest" ]; then
      echo "start to creat a soft link file for cantest"
      sudo ln -s ~/esdcan-pci200-linux-2.6.x-x86_64-3.10.1/bin64/cantest /usr/local/sbin/cantest
    fi
  fi

  if [ -z "$(ldconfig -p | grep esdcan-pci200)" ]; then
    echo "start to add the path of ~/esdcan-pci200-linux-2.6.x-x86_64-3.10.1/lib64/ into /etc/ld.so.conf "
    sudo sh -c "echo $HOME/esdcan-pci200-linux-2.6.x-x86_64-3.10.1/lib64/ >> /etc/ld.so.conf"
    echo "start to add the path of ~/esdcan-pci200-linux-2.6.x-x86_64-3.10.1/lib64/ into /etc/ld.so.cache"
    sudo ldconfig
  fi


  if [ -f "$HOME/esdcan-pci200-linux-2.6.x-x86_64-3.10.1/src/esdcan-pci200.ko" ]; then
    if [ -z "$(lsmod | grep esdcan_pci200)" ]; then 
      echo "start to insmod esdcan-pci200.ko"
      sudo insmod $HOME/esdcan-pci200-linux-2.6.x-x86_64-3.10.1/src/esdcan-pci200.ko
    fi
  fi
  ...
  if [ ! -L "/usr/local/sbin/cantest" ]; then
    if [ -f "$HOME/esdcan-pci331-linux-2.6.x-x86_64-3.10.1/bin64/cantest" ]; then
      echo "start to creat a soft link file for cantest"
      sudo ln -s ~/esdcan-pci331-linux-2.6.x-x86_64-3.10.1/bin64/cantest /usr/local/sbin/cantest
    fi
  fi

  if [ -z "$(ldconfig -p | grep esdcan-pci331)" ]; then
    echo "start to add the path of ~/esdcan-pci331-linux-2.6.x-x86_64-3.10.1/lib64/ into /etc/ld.so.conf "
    sudo sh -c "echo $HOME/esdcan-pci331-linux-2.6.x-x86_64-3.10.1/lib64/ >> /etc/ld.so.conf"
    echo "start to add the path of ~/esdcan-pci331-linux-2.6.x-x86_64-3.10.1/lib64/ into /etc/ld.so.cache"
    sudo ldconfig
  fi


  if [ -f "$HOME/esdcan-pci331-linux-2.6.x-x86_64-3.10.1/src/esdcan-pci331.ko" ]; then
    if [ -z "$(lsmod | grep esdcan_pci331)" ]; then
      echo "start to insmod esdcan-pci331.ko" 
      sudo insmod $HOME/esdcan-pci331-linux-2.6.x-x86_64-3.10.1/src/esdcan-pci331.ko
    fi
  fi
